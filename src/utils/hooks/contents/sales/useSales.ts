...
import {
  ...
  salesConsultProps,
  userCountListAction,
} from '~/stores/modules/contents/sales/sales'
import { regionListAction } from '~/stores/modules/contents/sales/sales'
...
import { useSalesConsult } from '~/utils/api/sales/useSalesConsult'
...
export const useSales = () => {
  ...
  const {
    ...
    agreeNoticeInfo,
    isActionButton,
  } = useAppSelector(state => state.salesSlice)
  ...
  const apiSalesConsult = useSalesConsult()

  const actionValidate = async (items: salesConsultProps) => {
    let isAction = true
    ...
    if (items.companyInfo.region.id === '') {
      openToast('국가를 선택해주세요.', 'warning')
      tempCompanyInfo.regionErr = '국가를 선택해주세요.'
      isAction = false
    }
  ...
  const salesConsult = async (items: salesConsultProps) => {
    ...
    if (check) {
      ...
      const params = {
        ...
        message: items.msgToNwire,
        infoGatherAgreed: !!find,
      }
      const { status, data, message } = await apiSalesConsult.mutateAsync(params)
      if (status === 'S') {
  ...
  const setCompanyInfoAddressNmAction = useCallback(
    async (e: string, hooks: companyInfoProps) => {
      const params = {
        ...hooks,
        addressNm: e,
  ...
  const setCompanyInfoSubAddressNmAction = useCallback(
    async (e: string, hooks: companyInfoProps) => {
      const params = {
        ...hooks,
        subAddressNm: e,
  ...
  const setMsgToNwireAction = useCallback(
    async (e: string) => {
      dispatch(msgToNwireAction(e))
    },
    [dispatch]
  )
