...
import { ContentItem } from '~/components/contents/common/forms/PublishingSearchForm/ContentItem'
...
interface Props {
  ...
  setKeywordPublishing: (e: MbTagSearchTagItem[]) => void
}

const PublishingSearch = (props: Props) => {
  ...
  const onChngeKeywordPublishing = async (i: boolean, e: SelectListOptionItem) => {
    ...
    props.setKeywordPublishing(dataList)
  }
              ...
              <>
                {props.originList &&
                  ...
                  props.originList.map(e => (
                    <ContentItem
                      key={'journalistOccupationList' + e.id + e.name + props.idKey}
                      idKey={props.idKey}
                      item={e}
                      tagItems={props.keywordParam}
                      onChangeChecked={(i, key) => onChngeKeywordPublishing(i, key)}
                    />
                  ))}
              </>
            </ul>
...
export default PublishingSearch
