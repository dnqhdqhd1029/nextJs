...
import { useSearchMediaGroup } from '~/utils/hooks/contents/pressMedia/useSearchMediaGroup'
...
const MediaGroupingForm = (props: Props) => {
  ...
  const className = 'mediaIdGroupList'

  // 미디어그룹 Hook
  const { setMediaGroup, setMediaGroupCount, handleMediaGroupAutoCompleteResult } = useSearchMediaGroup({
    tagSearchList,
    setTagSearchList,
    afterLoadComplete: async (data, countArrayName) => {
      ...
      for await (const mbTagSearchResultItem of data) {
        const checkedData = tagList.find(
          tag => (tag.id === mbTagSearchResultItem.name?.toString() ?? uuid()) && tag.className === className
        )
        const newResultItem = {
          id: mbTagSearchResultItem.name?.toString() ?? uuid(),
          label: `${mbTagSearchResultItem.name} - ${mbTagSearchResultItem.count}` ?? '',
          checked: !!checkedData,
        }
        if (checkedData) {
  ...
  const handleListScroll = (e: UIEvent<HTMLDivElement>) => {
    ...
    if (isBottomOfScroll && !isLoading && hasMoreItem) {
      setMediaGroupCount(prev => prev + AUTO_COMPLETE_COUNT)
      setTimeout(() => {
  ...
  const handleNameSearchChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    timerRef.current = setTimeout(async () => {
      setMediaGroup(value)
      setMediaGroupCount(AUTO_COMPLETE_COUNT)
    }, DEBOUNCE_DELAY_TIME)
  }
...
export default MediaGroupingForm
