...
import InputMediaList from '~/components/contents/common/email/MediaPopup/InputMediaList'
import { useEmail } from '~/utils/hooks/contents/email/useEmail'

const fileSizeUnit = 'MB'
const fileSizeLimit = 5

const InputMediaPopupImgContent = () => {
  const { inputMediaPopup, setInputMediaPopupSizeAction, setInputMediaFileImagePopupAction, mediaUploadFile } =
    useEmail()
        ...
        >
          <section className={cn('fui-dropzone-root', { 'is-disabled': isDragging })}>
            <div className="files-ui-dropzone-children-container">
              <label htmlFor="fileUpload_inputMediaPopup_img"> 클릭하거나 드래그해 파일을 올리세요.</label>
              <input
                id={'fileUpload_inputMediaPopup_img'}
                type="file"
          ...
          <>
            <div className="popup-file-list__group">
              <div className="popup-file-list__upload">
                <div className="file-uploader-list__section">
                  <ul className="file-uploader-list__area">
                    {inputMediaPopup.imageList.map((e, index) => (
                      <InputMediaList
...
export default InputMediaPopupImgContent
