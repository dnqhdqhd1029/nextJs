...
import MediaContentItem from '~/components/contents/pressMedia/SavedSearch/LeftContent/Media/MediaContentItem'
...
const MediaContents = () => {
  ...
  useEffect(() => {
    savedMediaListKeyword !== '' && getMediaListByKeyword(savedMediaListKeyword, isOwner, savedMediaKey)
        ...
        >
          <div className="accordion-type1-panel__group display-block">
            <ul className="accordion-type1-panel__option-list">
              {originSavedMediaList && originSavedMediaList.length >= 10 && (
                <div className="search">
                  <FormInputSearch
                    placeholder={'검색'}
                    value={savedMediaListKeyword}
                    onChange={e => setSavedMediaListKeywordAction(e.target.value, originSavedMediaList)}
                    onDeleteButtonClick={() => setSavedMediaListKeywordAction('', originSavedMediaList)}
                  />
                </div>
              )}
              {savedMediaList && savedMediaList.length > 0 && (
                <Fragment>
                  {savedMediaList.slice(0, 10 * isExpand).map(e => (
                    <MediaContentItem
                      key={'savedMediaList__accordion_category_accordion-type1' + e.mediaSrchId}
                      {...e}
                    />
                  ))}
                  {savedMediaList.length > isExpand * 10 && savedMediaList.length > 10 && (
                    <li>
                      <button
