...
import MediaSearchFilterItemSub from '~/components/contents/pressMedia/SavedSearch/LeftContent/Media/MediaSearchFilterItemSub'
...
const MediaSearchFilterItemWithSub = (props: Props) => {
  const {
    mediaListParams,
    isOwner,
    isFilterSubParam,
  ...
  const checkTotalValue = async () => {
    ...
    if (
      filterMediaSubParamActions &&
      props.item.subMenus &&
      filterMediaSubParamActions[props.keyValue] &&
      filterMediaSubParamActions[props.keyValue].values.length > 0
    ) {
      const tempFilterSubParam = [...filterMediaSubParamActions]
    ...
    <li id={props.item.id + 'lnb-filter__menu-list_subNewsFilterListListtype-submenu'}>
      {props.item.id && props.item.title && (
        ...
        >
          ...
          <span className="lnb-filter__menu-txt type-submenu">({props.item.subMenus?.length})</span>
...
export default MediaSearchFilterItemWithSub
