...
const PressHeader = () => {
  ...
  const checkActiveId = async () => {
    ...
    if (searchContentKeyList.length > 0) {
      if (searchContentKeyList.length === pageCount.totalCount) {
        ...
        if (pressContentList.length > 0) {
          for await (const eElement of pressContentList) {
            const temp = searchContentKeyList as pressContentListProps[]
            const find = temp.find(e => e?.jrnlstListId === eElement?.jrnlstListId)
            if (find) {
              count += 1
            }
          }
          if (pressContentList.length === count) {
            checkStatus = true
          }
        }
      }
  ...
  const checkedAll = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setAllSearchContentKeyList(e.target.checked, pressContentList, searchContentKeyList)
  }
  ...
  const handleShareCode = (e: SelectListOptionItem) => {
    setIsOption(() => false)
    handleChangeShareCode(e, pressListParams)
  }
                ...
                <li className="button">
                  <div className="select__section select-type1-small select-line select-align-right">
                    <Button
                      ...
                      size={'m'}
                      color={'primary'}
                      onClick={() => setPressGroupPopupAction()}
                    />
                  </div>
                </li>
              </ul>
          ...
          <li>
            <div className="search-result__header-sort type-pl-14">
              <FormInputBtn
                ...
                checked={isSelected}
                onChange={e => pressContentList.length > 0 && checkedAll(e)}
              />
                ...
                <ul className="s-header__control">
                              ...
                              <li key={'defaultSelectLabel_monitoring_management' + e.id}>
                                <button
                                  className={cn('select-option__item', {
                                    'is-selected': pressListParams.shareCode.id === e.id,
                                  })}
                  ...
                  <li className="filter">
                    <Button
                      ...
                      icoSize={18}
                      onClick={() => setManagementContentListButtonAction(!pressContentListButton)}
                    />
                    {pressListParams.sort && pressListParams.sort.length > 0 && (
                      <SortFilterList
                        sortOptionsByData={defaultSortOptionsByData}
                        onChange={(dataItem: SelectListOptionItem, orderItem: SelectListOptionItem) =>
                          handleChangeSort([`${dataItem.id}!${orderItem.id}`], pressListParams)
                        }
                        value={pressListParams.sort as string[]}
