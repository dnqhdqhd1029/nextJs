...
const ShareItem = (props: pressContentListProps) => {
  const { shareCodeChangeAction } = usePressListManagement()
  ...
  return (
    <li
      ...
      id={'list-type4-item__share-filter' + props.jrnlstListId}
      ref={shareOpenRef}
    >
              ...
              {DefaultShareFilterOptionList.map(e => (
                <li key={'DefaultShareFilterOptionList' + e.id + e.name}>
                  <button
                    ...
                    onClick={() => shareCodeChangeAction(Number(props.jrnlstListId), e.id, props.title)}
                  >
