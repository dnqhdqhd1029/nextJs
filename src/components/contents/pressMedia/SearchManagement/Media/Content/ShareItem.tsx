...
import { DefaultShareFilterOptionList } from '~/components/contents/pressMedia/SearchManagement/defaultData'
import { mediaContentListProps } from '~/stores/modules/contents/pressMedia/mediaSearchManagement'
...
const ShareItem = (props: mediaContentListProps) => {
  const { shareCodeChangeAction } = useMediaSavedSearchManagement()
  ...
  return (
    <li
      ...
      id={'list-type4-item__share-filter' + props.contact_id}
      ref={shareOpenRef}
    >
              ...
              {DefaultShareFilterOptionList.map(e => (
                <li key={'DefaultShareFilterOptionList' + e.id + e.name}>
                  <button
                    ...
                    onClick={() => shareCodeChangeAction(Number(props.contact_id), e.id)}
                  >
