...
const ExcelFileDownloadPopup = () => {
  const { fileDownloadPopup, pageCount, searchContentKeyList, setSelectedExcelFileData, exportToExcel } =
    useActivityList()
  ...
  const activityAction = async (e: MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault()
    setIsLoading(() => true)
    await exportToExcel(searchContentKeyList)
