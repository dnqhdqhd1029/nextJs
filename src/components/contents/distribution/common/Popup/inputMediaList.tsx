...
import { inputMediaPopupType } from '~/stores/modules/contents/newswireRelease/newswireRelease'
...
interface Props {
  ...
  setInputMediaFileCheckPopupAction: Function
  setInputMediaImageCheckPopupAction: Function
  setDeleteInputMediaImagePopupAction: Function
}
const InputMediaList = (props: Props) => {
  ...
  const checkedAction = async () => {
    if (item.id) {
      inputMediaPopup.radioSelected !== 'IMG'
        ? setInputMediaFileCheckPopupAction(item, isChecked, inputMediaPopup, inputMediaPopup.releaseFilesItems)
        : setInputMediaImageCheckPopupAction(item, isChecked, inputMediaPopup, inputMediaPopup.releaseImageItems)
    }
  }
  ...
  useEffect(() => {
    if (item.id && inputMediaPopup.radioSelected === 'IMG') getImage()
  }, [item.id])
        ...
        <li>
          <div className={`file-uploader-button-list__item ${isChecked ? 'is-selected' : ''}`}>
            ...
            <Button
              ...
              icoSize={16}
              onClick={() => setDeleteInputMediaImagePopupAction(Number(item.id))}
            />
          </div>
        </li>
      ) : (
          ...
          <div className="file-uploader-list__group">
            <div className="file-uploader-list-item__section">
              ...
              <p className="file-uploader-list-item__text">
                {item.width} X {item.height} ({getCurrencyFormat(Math.round(item.size ?? 0))}KB)
              </p>
