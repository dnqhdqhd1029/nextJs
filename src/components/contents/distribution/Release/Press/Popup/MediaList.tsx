...
const MediaList = (props: Props) => {
  const { setEditorMediaFileCheckPopupAction, setEditorMediaImageCheckPopupAction, setDeleteMediaImagePopupAction } =
    props
  const [loading, setLoading] = useState<boolean>(false)
  const [isChecked, setIsChecked] = useState<boolean>(false)
  const [imageSrc, setImageSrc] = useState<string>('')
  const { userSelectGroup } = useAppSelector(state => state.authSlice)
  const { mediaPopup } = useAppSelector(state => state.pressReleaseSlice)
  ...
  useEffect(() => {
    if (props.item.id && mediaPopup.radioSelected === 'IMG') getImage()
  }, [props.item.id])
        ...
        <li>
          <div className={`file-uploader-button-list__item ${isChecked ? 'is-selected' : ''}`}>
            ...
            <p
              ...
              onClick={() => checkedAction()}
            >
              {getCurrencyFormat(Math.round(props.item.size ?? 0))}KB
            </p>
            <Button
              label={'삭제'}
              cate={'ico-only'}
              ...
              icoSize={16}
              onClick={() => setDeleteMediaImagePopupAction(Number(props.item.id))}
            />
          </div>
        </li>
      ) : (
