...
import { DefaultShareFilterOptionList } from '~/components/contents/monitoring/Management/defaultData'
...
const ShareItem = (props: managementContentListProps) => {
  const { managementListParams, shareCodeChangeAction } = useMonitoringManagement()
  ...
  return (
    <li
      ...
      id={'list-type4-item__share-filter' + props.newsSrchId}
      ref={shareOpenRef}
    >
              ...
              {DefaultShareFilterOptionList.map(e => (
                <li key={'DefaultShareFilterOptionList' + e.id + e.name}>
                  <button
                    ...
                    onClick={() => shareCodeChangeAction(Number(props.newsSrchId), e.id, managementListParams)}
                  >
