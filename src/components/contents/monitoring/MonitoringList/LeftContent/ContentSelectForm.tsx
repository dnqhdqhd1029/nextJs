...
import SelectItems from '~/components/contents/monitoring/MonitoringList/LeftContent/SelectItems'
import { useMonitoringSearch } from '~/utils/hooks/contents/monitoring/useMonitoringSearch'

const ContentSelectForm = () => {
  const { monitoringList, monitoringCategoryData } = useMonitoringSearch()
  ...
  return (
    ...
    >
      <div className="select-form__group">
        {monitoringList.length < 1 && monitoringList.length < 1 ? (
          ...
          >
            <span className="select-form__label-text">{monitoringCategoryData?.title}</span>
          ...
          <div className="select-form-option__area">
            {monitoringList &&
              ...
              monitoringList.map(e => (
                <Fragment key={'monitoringList__aselect-form-option__area_group-title' + e.categoryNm}>
                  ...
                  <ul className="select-form-option__group">
                    {e.content &&
                      ...
                      e.content.map(i => (
                        <SelectItems
                          key={'monitoringList__select-form-option-type1' + i.newsSrchId}
                          {...i}
                        />
                      ))}
                  </ul>
